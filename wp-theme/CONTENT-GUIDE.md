...
- Bento Grid's cards repeat a 2-big/2-small size pattern automatically based on position, no matter how many cards you add. The **Card Size Pattern** select controls which size comes first: `2 Big cards, then 2 Small` (default — 1&2 big, 3&4 small, 5&6 big, 7&8 small...) or `2 Small cards, then 2 Big` (1&2 small, 3&4 big, 5&6 small, 7&8 big...). Each card also has an optional **Link** field — set it and the whole card becomes clickable. There's also a **Show Button** toggle (with **Button Label** / **Button Link**) for a single button centered below the grid, e.g. "See More".
- There's an **FAQs (Accordion)** layout for a collapsible question/answer list — add rows under **Questions** (Question + Answer pairs, at least 1 required). The first question is open by default; clicking any question opens it and closes whichever one was open, so only one is ever expanded. Like Bento Grid, it has an optional **Show Button** toggle (**Button Label** / **Button Link**) for a "See More" button centered below the list — leave it off if the page doesn't need one.
- The header's "aik connect" pill button and the floating social icons (bottom-right, every page) aren't Page Sections fields — they're part of the shared header, always on. The floating icons' Facebook/Twitter/Instagram/WhatsApp links come from **Theme Settings** (same fields the footer's social icons use — set once, both places update). The 5th icon (chat/robot) is a placeholder, not wired to anything yet.
- **"Media News" is not a Page** — every News item is a normal WordPress **Post** (Posts → Add New), same as the News section on the homepage already pulls from (see the News layout note above). Publish a post with a Featured Image, Title, and body content, and it automatically appears in both places: the homepage's News section preview and the full **Media News** listing at whatever URL Settings → Reading's **Posts page** is set to (see README.md step 8 — that setting must be filled in, or "See More" has nowhere real to link to). That listing is rendered by `home.php` (a 4-column grid) — note this is `home.php`, not `archive.php`; the "Posts page" is a distinct template slot in WordPress's hierarchy from category/tag archives, which is what `archive.php` is actually for. Both share the same markup via `template-parts/news-archive-content.php`, so they can't drift apart. This is the pattern to follow for anything similar in the future (blog, press releases, etc.) — use Posts + Categories, not a custom Page, whenever it's a list of dated items that also need their own detail page.
- **Not built yet:** the individual article page (what "Read More" links to) — there's no `single.php`, so clicking through to read a full post currently falls back to a generic/unstyled template. Needs its own pass matching the site design before Media News is fully usable end-to-end.
...
- **App Download (Band Design + Mobile)** also has a **Show Help Search** toggle — when on, it adds a "How May We Help You?" heading + search bar directly above the band, sharing its background with no gap. The heading text is editable (**Help Search Heading**); the search box submits to the site's built-in WordPress search. Off by default — only turn it on where the original design calls for it (e.g. Home).
...
- Category: News — Number of Posts: *(leave blank — shows every post in the category automatically)*
...
- Heading: `Our Media News`
- Description: `Stay informed with the latest updates, expert insights, and practical guides on digital finance and Islamic banking. From everyday money tips to Shariah-compliant financial knowledge, explore content designed to help you make smarter, more ethical financial decisions.`
- Category: News — Number of Posts: *(leave blank — shows every post in the category automatically, same News posts created for the Home page)*
